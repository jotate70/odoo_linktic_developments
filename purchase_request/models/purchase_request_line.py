...
class PurchaseRequestLine(models.Model):
    ...
    state = fields.Selection(selection=[('draft', 'Draft'), ('to_approve', 'To Approve'), ('approved', 'Approved'),
                                        ('rejected', 'Rejected'), ('done', 'Done'), ('closed', 'Closed')]
                             , string="Status", index=True, tracking=True, required=True, copy=False, default="draft")
    quotation_ids = fields.One2many(comodel_name="purchase.request.quotation", inverse_name="purchase_request_line_id",
                                    string="Purchase Request Quotations", copy=False)
    general_budget_id = fields.Many2one('account.budget.post', 'Budgetary Position')
    budget_line_segregation_id = fields.Many2one('crossovered.budget.lines.segregation', 'Budget Line Segregation',
                                                 domain="[('general_budget_id', '=?', general_budget_id), ('analytic_account_id', '=?', analytic_account_id)]")
    segregation_balance = fields.Monetary(string="Balance", compute="get_segregation_balance", store=True,
                                          readonly=True)
    purchase_policy_modification = fields.Boolean(string='Policy Modification', default=False)
    purchase_policy_order_id = fields.Many2one(comodel_name="purchase.order", string='Policy Purchase', copy=False
                                               , domain="[('project_id', '=', parent.project_id), ('project_id', '!=', False)]")
    ...
    @api.onchange('purchase_policy_modification')
    def _onchange_purchase_policy_modification(self):
        for record in self:
            if not record.purchase_policy_modification:
                record.purchase_policy_order_id = False
    ...
    def onchange_product_id(self):
        if self.product_id:
            ...
            self.budget_line_segregation_id = False
    ...
    def action_open_line_quotations(self):
        self.ensure_one()
        if self.request_id.state in ("draft", "to_approve"):
            raise UserError(_("Cant manage quotations of a non approved request"))
        action = self.env['ir.actions.act_window']._for_xml_id('purchase_request.purchase_request_quotation_action')
        action['domain'] = [('purchase_request_line_id', '=', self.id)]
        action['context'] = {'default_purchase_request_line_id': self.id}
        return action
