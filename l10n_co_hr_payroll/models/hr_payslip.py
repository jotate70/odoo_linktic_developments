...
class HrPayslip(models.Model):
    ...
    def get_json_request(self):
        for rec in self:
            if not rec.number:
                raise UserError(_("The payroll must have a consecutive number, 'Reference' field for the employee %s." % rec.employee_id.name))
            if not rec.contract_id.payroll_period_id:
                raise UserError(_("The contract must have the 'Scheduled Pay' field configured for the employee %s." % rec.employee_id.name))
            ...
            if not rec.company_id.type_document_identification_id:
                raise UserError(_("Your company does not have an identification type for the employee %s." % rec.employee_id.name))
            if not rec.company_id.vat:
                raise UserError(_("Your company does not have a document number for the employee %s." % rec.employee_id.name))
            if not rec.company_id.partner_id.postal_municipality_id:
                raise UserError(_("Your company does not have a postal municipality for the employee %s." % rec.employee_id.name))
            ...
            if not rec.contract_id.type_worker_id:
                raise UserError(_("The contract must have the 'Type worker' field configured for the employee %s." % rec.employee_id.name))
            if not rec.contract_id.subtype_worker_id:
                raise UserError(_("The contract must have the 'Subtype worker' field configured for the employee %s." % rec.employee_id.name))
            ...
            if not rec.employee_id.address_home_id.type_document_identification_id:
                raise UserError(_("Employee does not have an identification type for the employee %s." % rec.employee_id.name))
            if rec.employee_id.address_home_id.type_document_identification_id.id == 6:
                raise UserError(_("The employee's document type cannot be NIT for the employee %s." % rec.employee_id.name))
            if not rec.employee_id.address_home_id.vat:
                raise UserError(_("Employee does not have an document number for the employee %s." % rec.employee_id.name))
            if not rec.employee_id.address_home_id.postal_municipality_id:
                raise UserError(_("Employee does not have a postal municipality for the employee %s." % rec.employee_id.name))
            ...
            if rec.contract_id.wage <= 0:
                raise UserError(_("The contract must have the 'Wage' field configured for the employee %s." % rec.employee_id.name))
            if not rec.contract_id.type_contract_id:
                raise UserError(_("The contract must have the 'Type contract' field configured for the employee %s." % rec.employee_id.name))
            if not rec.contract_id.date_start:
                raise UserError(_("The contract must have the 'Start Date' field configured for the employee %s." % rec.employee_id.name))
